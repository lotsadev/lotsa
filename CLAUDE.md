...
| 044 | Workflows for agents — generic `AGENT_RESULT:` outcome vocabulary (`COMPLETED`/`PASSED`/`FAILED`/`SKIPPED`/`INPUT`) + a shared, process-independent agent catalog (`lotsa/prompts/agents/`); routing moves from marker name to flow edge (**Implemented — Phases 1–5 & 6-viewer; Phase 6 editor deferred**; Phase 2 wires `produces_changes` → derived `commit` posthook in `flows.py`, dropping hand-declared `posthooks: [commit]` from bundled `build`/`fix` and making `verify` no longer commit; Phase 3 wires `needs_worktree` → derived `worktree` prehook (opt-out — worktree is the universal default, only `chat` opts out), moving worktree creation off unconditional dispatch onto `_run_step_prehooks`; Phase 4 adds the `routes:` routing sugar (`{OUTCOME: target}` desugared into `^AGENT_RESULT:` `OutputRule`s; bundled `build`/`fix` migrated behaviour-identically) + a gate-only derived `FAILED → blocked` default, and the `chat` de-special-casing via a declared `invocable: [start | hand-off]` property replacing `name == "chat"` checks (drops the hard promote-into-chat block — amends ADR-027 §7); Phase 4c's promotion payload also shipped — the `handoff` edge captures a recommended-workflow (`AGENT_RESULT: COMPLETED <workflow>`) into a non-terminating latest-wins `handoff_suggestion` artifact (promote dialog pre-selects it) and the distilled spec carries to Execute as `draft_spec` via `promotion_inputs`, with promotion itself operator-driven (ADR-027); only a single bundled payload struct was declined as an unnecessary nicety. Phase 5 lands git-native `.lotsa/` provenance — a repo ships agents (`<repo>/.lotsa/agents/`) and workflows (`<repo>/.lotsa/workflows/`) discovered from the project root (`lotsa/provenance.py`, project-scoped/build-time), a namespace-aware `AgentPromptRegistry` (unqualified resolves operator-override → `lotsa:` → `repo:`, repo lowest-trust; `lotsa:`/`repo:` qualifiers bind explicitly), per-project `self._project_processes` (project-isolated, fail-soft, cannot shadow bundled), with structural rails (always-injected preamble, deterministic push, bundled-tools-only references, name-charset + `.lotsa`-containment/symlink guards); Phase 6 (viewer) ships a read-only workflow graph viewer — `flows.serialize_process_graph` turns a resolved `Process` into per-flow nodes (resolved agent + `class`/`outcomes`/hooks) + edges (per-binding routes, implicit forward edge, materialized `blocked`/`needs_input`/`complete` sinks), `OrchestratorService.workflow_graph`/`agent_detail` add provenance-derived `source` (`repo` vs `bundled`, from the `_processes` vs `_project_processes` split; also on `list_processes_summary`), `GET /api/workflows/{name}/graph` + `/agents/{prompt_name}` (project-scoped), and a header-launched React Flow (`@xyflow/react`) + Dagre (`@dagrejs/dagre`) viewer surface (workflow list + provenance badge, board of shadcn nodes, node-click agent inspector, main/pr_fix flow selector); the editor (write affordances, DB-backed storage) stays deferred behind the props-flip seam. Amends ADR-043/039/014/027 | CE |

| 045 | Workflows call workflows — drop sub-flows; every workflow is top-level and callable (**Implemented — Phase 1**: a persisted `call_stack` of `{workflow, step, called_from}` frames (reserved-but-absent `vars` slot) replaces the single `current_flow` string, push/pop via ADR-020 CAS (`lotsa/call_stack.py`); `routes: { COMPLETED: call <workflow>[@<step>] }` is a YAML-only static call edge — unknown target or cycle fails at process-build time via `flows.validate_call_graph`, and an agent-chosen `CALL` form is rejected; `complete` pops one frame and returns to the caller's next step, `terminate` unwinds the stack subject to per-frame `routes: { terminate: <catch> }` catches (an uncaught `terminate` at root ends the task via the untouched ADR-030 terminal CAS — `terminate` *sits beside* ADR-043 `mark_complete` and ADR-030 terminal PR signals, not replacing either); `pr-monitor` is extracted as a standalone bundled workflow with `wait_for_pr_signal` as its entry step and the `SKIPPED → wait_for_pr_signal` edge internal, so `build`/`fix` mains end at `push_pr → call pr-monitor` (automatic/ungated) with both duplicate `pr_fix` flows deleted; the `flows.py` cross-flow SKIPPED special case + `_register_cross_flow_edges` and the orchestrator `current_flow`-reset drainer branches are removed; `dispatch_sub_flow` completes Layer B — dispatches into any named workflow and honours `target_job` for named re-entry, the `flow_name != "pr_fix"` rejection gone; APPROVED parks at the in-`pr-monitor` operator gate and keeps monitoring (only merged/closed reach `terminate`); `chat → build` still uses ADR-027 promotion (no ungated call into a code-writing workflow); repo-shipped workflows with an invalid `call` graph fail-soft — the whole project catalog is dropped, never partially registered. **Legacy `current_flow` rows are not migrated** — a pre-stack row routes to `blocked` (pre-alpha break; close/complete in-flight tasks before upgrading — see ADR implementation notes). Phases 2–4 (call-edge `gate: operator`, standalone pr-monitor against a foreign PR, viewer call edges) + the metadata/variable layer deferred.) | CE |

| 046 | Standing monitors & the heartbeat protocol — extracts the latent monitor framework into a first-class `Monitor` base + `MonitorHeartbeat`/`MonitorRegistry` liveness (`lotsa/monitors/`; interval loop, exception isolation, `aclose()` teardown, per-tick heartbeat; `kind` = `standing` \| `step_scoped`); `pr_monitor` migrated onto the base behaviour-preservingly; a standing project-wide **branch-freshness** monitor (`lotsa/branch_monitor.py`) as consumer #1 — one `git fetch origin/<default>` per project per interval, then a **non-mutating** `git rev-list` + `git merge-tree --write-tree` probe (Git ≥2.38, `branch_mergeable=null` fallback) persisting `branch_behind`/`branch_mergeable`/`branch_conflicts`/`branch_checked_at` to task metadata for non-terminal worktree-bearing tasks; dashboard sidebar "↓N behind" badge + Changes-tab strip + idle-gated **Sync with default** button (`POST /tasks/{id}/sync-branch` → `sync_branch` reuses `_sync_branch_to_main(push=…)`: push when a PR exists, local merge pre-PR; raced conflict → `_handle_conflict_dispatch` → `resolve_conflicts`); `GET /api/monitors` liveness read; `branch_monitor_interval_seconds` (default 300) + `branch_monitor_enabled` knobs. Polling cousin of ADR-044's event hooks (**Implemented** — `lotsa doctor` check + dashboard liveness strip deferred) | CE |
