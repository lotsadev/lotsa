...
| 044 | Workflows for agents — generic `AGENT_RESULT:` outcome vocabulary (`COMPLETED`/`PASSED`/`FAILED`/`SKIPPED`/`INPUT`) + a shared, process-independent agent catalog (`lotsa/prompts/agents/`); routing moves from marker name to flow edge (**Implemented — Phases 1 & 2**; Phase 2 wires `produces_changes` → derived `commit` posthook in `flows.py`, dropping hand-declared `posthooks: [commit]` from bundled `build`/`fix` and making `verify` no longer commit. Phases 3–6 proposed: `needs_worktree` prehook, workflow-model cleanup, git-native `.lotsa/` provenance, visual editor). Amends ADR-043/039/014 | CE |
