...
def append_lotsa_trailer(body: str, task_id: str, flow_name: str = "") -> str:
    """Append the ``_Generated by Lotsa · task <id> · flow <name>_`` trailer.

    Owned by the push path (not the agent) so ``task_id``/``flow_name`` stay
    authoritative and the trailer is appended exactly once regardless of
    whether the body came from the agent artifact or the fallback generator.
    """
    flow_part = f" · flow {flow_name}" if flow_name else ""
    trailer = f"_Generated by Lotsa · task {task_id}{flow_part}_"
    body = (body or "").rstrip()
    if not body:
        return trailer
    return f"{body}\n\n---\n\n{trailer}"
...
async def build_pr_text(
    *,
    ...
    appended exactly once.  ``spec`` is last-resort context for the fallback —
    the diff and commit messages are the ground truth.
    """
    ...
    return title, append_lotsa_trailer(body, task_id, flow_name)
