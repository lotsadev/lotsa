...
def test_append_lotsa_trailer_includes_process_name():
    """The trailer names the process so a PR is debuggable (build vs fix).

    ``flow_name`` is always ``main``/``pr_fix`` and structurally cannot tell the
    processes apart. Fails pre-fix: ``append_lotsa_trailer`` has no
    ``process_name`` parameter (TypeError)."""
    from lotsa.push_step import append_lotsa_trailer

    out = append_lotsa_trailer("The body.", "task-9", "main", "build")
    assert "The body." in out
    assert "Generated by Lotsa" in out
    assert "task task-9" in out
    assert "process build" in out
    assert "flow main" in out


def test_append_lotsa_trailer_omits_process_when_empty():
    """An empty ``process_name`` drops the process segment (back-compat)."""
    from lotsa.push_step import append_lotsa_trailer

    out = append_lotsa_trailer("Body", "task-9", "main", "")
    assert "Generated by Lotsa" in out
    assert "· process " not in out


def test_append_lotsa_trailer_still_added_once_with_process():
    from lotsa.push_step import append_lotsa_trailer

    out = append_lotsa_trailer("Body", "task-1", "main", "fix")
    assert out.count("Generated by Lotsa") == 1
