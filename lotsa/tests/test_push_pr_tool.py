...
def test_push_pr_threads_process_name_into_trailer(tmp_path):
    """``task.process_name`` reaches the Lotsa PR trailer end-to-end.

    ``TaskContext`` already carries ``process_name``; the tool must thread it
    through ``build_pr_text`` → ``append_lotsa_trailer`` so the created PR body
    reads ``· process <build|fix>``. A valid ``pr_description`` parses cleanly,
    so ``build_pr_text`` short-circuits the git collectors (no repo needed).

    Fails pre-fix: ``push_pr`` never passes ``process_name`` to
    ``build_pr_text``, so ``build_pr_text`` cannot inject it — the assertion on
    ``· process fix`` in the body fires (and pre-fix ``append_lotsa_trailer``
    would ``TypeError`` if handed the kwarg at all)."""
    from lotsa.tools import TaskContext
    from lotsa.tools.push_pr import push_pr

    db = _FakeDB(artifacts={"pr_description": "docs: update readme\n\nClarify setup."})
    ctx = TaskContext(
        task_id="task-001",
        worktree=tmp_path,
        metadata={},  # no pr_number → PR-creation path → build_pr_text runs
        db=db,
        process_name="fix",
        flow_name="main",
        current_flow="main",
    ...
    assert result.success is True
    body = mock_push.call_args.kwargs.get("body") or ""
    assert "Generated by Lotsa" in body
    assert "· process fix" in body
